...
class MMDCritic:
    ...
    def select_criticisms(self, n, protos, train_indices, regularization:str | None="logdet", argMax:bool | None = True):
        """
        ...
        if train_indices is None :
            ...
            candidates = np.setdiff1d(train_indices, proto_indices)
        colsum = np.sum(self.crit_K, axis=0)/num_dp

        for _ in range(n):
            candidates = np.setdiff1d(candidates, selected)
            scores = colsum[candidates] - np.sum(self.crit_K[proto_indices, :][:, candidates], axis=0)/len(protos)

            if regularization == "logdet":
                if inverse_of_prev_selected is not None:
                    regularizer = np.log(np.abs(np.diagonal(self.crit_K)[candidates] - np.sum(np.array(np.dot(inverse_of_prev_selected, self.crit_K[selected, :][:, candidates])) * self.crit_K[selected, :][:, candidates], axis=0)))
                    scores = scores + regularizer
                else :
            ...
            if argMax :
                ...
                selected = np.append(selected, candidates[np.argmin(scores)])
            

            if regularization == "logdet":
                inverse_of_prev_selected = np.linalg.inv(self.crit_K[selected,:][:,selected])
